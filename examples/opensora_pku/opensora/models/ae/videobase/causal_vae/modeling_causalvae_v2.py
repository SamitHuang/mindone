...
from ..modules.attention import AttnBlock3D
from ..modules.conv import Conv2d, CausalConv3d
from ..modules.normalize import Normalize 
from ..modules.ops import nonlinearity
from ..modules.resnet_block import ResnetBlock2D, ResnetBlock3D
from ..modules.updownsample import SpatialDownsample2x, SpatialUpsample2x, TimeDownsample2x, TimeUpsample2x, Downsample, TimeDownsampleRes2x, TimeUpsampleRes2x 
# from ..utils.module_utils import  get_obj_from_str
    ...
    def __init__(
        ...
        **ignore_kwargs,
        ...
        hidden_size_mult: channel multiply factors for each res block, also determine the number of res blocks.
            Each block will be applied with spatial downsample x2 except for the last block. In total, the spatial downsample rate = 2**(len(hidden_size_mult)-1)
...
class Decoder(nn.Cell):
    ...
    def __init__(
        ...
        **ignore_kwargs,
        ...
        # 2. middle
        self.mid = nn.Cell()
        self.mid.block_1 = get_obj_from_str(mid_resnet)(in_channels=block_in, out_channels=block_in, dropout=dropout, dtype=self.dtype)
        self.mid.attn_1 = get_obj_from_str(attention)(block_in, dtype=self.dtype)
        self.mid.block_2 = get_obj_from_str(mid_resnet)(in_channels=block_in, out_channels=block_in, dropout=dropout, dtype=self.dtype)
        self.mid.update_parameters_name(prefix=self.param_prefix + "mid.")
