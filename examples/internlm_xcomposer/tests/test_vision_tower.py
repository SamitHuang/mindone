...
sys.path.append(mindnlp_lib_path)
...
def test(mode=0, jit_level=None, pt_npy=None):
    ...
    images = [ms.Tensor(np.load('tests/pt_save/images_0.npy'))]
    plora_glb_GN = np.load('tests/pt_save/plora_glb_GN.npy')
    plora_sub_GN = np.load('tests/pt_save/plora_sub_GN.npy')
...
if __name__ == "__main__":
    test(0, 'O0', 'tests/pt_save/pt_vision_tower_out.npy')
