...
def get_mm_param_prefix_indices(pt_mm_path, 
                     prefix_order = ['down_blocks', 'mid_block', 'up_blocks'], 
                     num_params_per_mm = 588//21):
...
def ms_extract_mm_params_by_index(mm_ckpt_path, 
                                  ms_pinfo_path, 
                                  start_idx=0, 
                                  num_params=588//21, 
                                  remove_prefix=True,
...
def test():
    ...
    mm_prefix, mm_prefix_shape, mm_param_start_idx = get_mm_param_prefix_indices(pt_mm_path, num_params_per_mm=num_params_per_mm)
    ...
    for mm_idx in range(num_mm):
        ...
        print(f'Idx: {mm_idx}, c: {in_ch}, hw: {ftr_hw}, param_prefix: {mm_prefix[mm_idx]}, param_start_idx: {mm_param_start_idx[mm_idx]}, param_shape: {mm_prefix_shape[mm_idx]}')
        ...
        # load mm weights
        cur_state_dict = ms_extract_mm_params_by_index(ms_mm_path, ms_pinfo_path, mm_param_start_idx[mm_idx], num_params=num_params_per_mm)
