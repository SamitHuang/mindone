...
def convert_pt_ms_state_dict(lora_ckpt_path, pt_pinfo_path, ms_pinfo_path, add_ldm_prefix=False, ldm_prefix='model.diffusion_model.', output_dir='./'):
    '''
    e.g.
    ...
    for pt_lora_pname  in pt_lora_pnames:
        ...
        pt_attn_pname = pt_lora_pname.replace("processor.", "").replace("_lora", "").replace("down.", "").replace("up.", "")
        ...
        assert pt_pshapes[tar_idx]==ms_pshapes[tar_idx], f"pt and ms param shape mismatch, please check order correspondance. pt: {pt_pname}, {pt_pshape}, ms: {ms_pname}, {ms_pshape}. "
...
if __name__ == '__main__':
    ...
    parser.add_argument("--target", type=str, default="../models/MotionLoRA", help='target dir to save the converted checkpoint')
