...
def read_gif(gif_path, mode='RGB'):
    with Image.open(gif_path) as fp:
        frames = np.array([
            ...
            for frame in ImageSequence.Iterator(fp)
        ])
    return frames
...
class TextVideoDataset():
    def __init__(
        ...
        self.pixel_transforms = create_video_transforms(sample_size[0], sample_size[1], sample_n_frames,  interoplation='bicubic', backend=transform_backend)
...
def check_sanity(x, save_fp='./tmp.gif'):
    ...
    imageio.mimsave(save_fp, x, duration=1/8., loop=1)
