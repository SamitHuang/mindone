...
class NonInflatedGroupNorm(nn.GroupNorm):
    '''
    compute GroupNorm in 5D based on the input in shape (b*f c h w) using the same way defined in AnimateDiff when use_inflated_groupnom=False, i.e. reshape to 5d tensor and normalize and reshape back  
