...
class ResBlock(nn.Cell):
    ...
    def __init__(
        ...
        '''
        norm_in_5d: If True, normalize the input of shape (b c f h w), i.e. reduce mean and variance to the first 2 axis (b c), which is equal to use_inflated_groupnorm=False in torch AnimateDiff. 
                If false, normalize the input of shape (b*f c h w), i.e. reduce mean and var to the first 2 axis (b*f c), equal to use_inflated_groupnorm=True. 
        '''
        super().__init__()
        self.channels = channels
        ...
        self.in_layers_norm = normalization(channels, norm_in_5d=norm_in_5d) # TODO: this is group norm actually, wrong naming. but renaming requires update of ckpt param name or mapping dict.
