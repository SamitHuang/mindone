...
def parse_args():
    ...
    parser.add_argument(
        "--betas", type=float, default=[0.9, 0.999], help="Specify the [beta1, beta2] parameter for the AdamW optimizer."
    )
    parser.add_argument(
        "--optim_eps", type=float, default=1e-6, help="Specify the eps parameter for the AdamW optimizer."
    )
    parser.add_argument(
        "--group_strategy", type=str, default='norm_and_bias', help="Grouping strategy for weight decay. If `norm_and_bias`, weight decay filter list is [beta, gamma, bias]. If None, filter list is [layernorm, bias]. Default: norm_and_bias"
    )
    ...
    parser.add_argument("--epochs", default=10, type=int, help="epochs. If dataset_sink_mode is on, epochs is with respect to dataset sink size. Otherwise, it's w.r.t the dataset size.")
    ...
    parser.add_argument("--save_mm_only", default=False, type=str2bool, help="If True, save motion module params only to reduce checkpoint size. Otherwise, save the whole ldm model (vae, clip, unet, mm) in one checkpoint")
    # video
    parser.add_argument("--image_finetune", default=True, type=str2bool, help="True for image finetune. False for motion module training.")
