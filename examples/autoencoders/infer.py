...
from ae.data.image_dataset import ImageDataset, create_dataloader 
...
def main(args):
    ...
    for step, data in tqdm(enumerate(ds_iter)):
        ...
        if not args.encode_only:
            ...
            ssim_cur = [calc_ssim(x_rgb[i], recons_rgb[i], data_range=255, channel_axis=-1,  multichannel=True) for i in range(x_rgb.shape[0])]
...
def parse_args():
    ...
    parser.add_argument("--csv_path", default=None, type=str, help="path to csv annotation file. If None, will get images from the folder of `data_path`")
    ...
    parser.add_argument("--eval_loss", default=False, type=str2bool, help="whether measure loss including reconstruction, kl, perceptual loss")
