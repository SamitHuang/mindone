...
def inject_trainable_lora(
    ...
    Args:
        net: input network
        target_modules: indicate the target types of cell modules for lora injection, e.g. ldm.modules.attention.CrossAttention. Complete invokable path is required. 
        target_layers: target layer names (defined inside the target modules) to be injected with LoRA dense layers, e.g. to_q. If the target layer is in a CellList or SequentialCell, add `[index]` after the layer name to specify the layer index.
        rank: lora rank
    ...
    for sc_name, subcell in catched_attns.items():
        ...
        for i, layer_name in enumerate(target_layers):
            ...
            if not isinstance(tar_dense, ms.nn.Dense):
                raise ValueError(f"{tar_dense} is NOT a nn.Dense layer, currently only support lora injection to Dense layers")
