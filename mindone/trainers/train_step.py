    ...
    def construct(self, *inputs):
        ...
        if (not self.drop_overflow_update) or (not overflow):
            ...
            if self.accum_steps > 1:
                ...
                loss = F.depend(loss, self.hyper_map(F.partial(_grad_accum_op, self.accum_steps), self.accumulated_grads, grads))
                ...
                if self.cur_accum_step >= self.accum_steps:
                    ...
                    loss = F.depend(
                        loss, self.hyper_map(F.partial(_grad_clear_op), self.accumulated_grads))
